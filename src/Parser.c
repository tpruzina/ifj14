	...
	switch((*cur)->type){
		...
		case T_ID:
			...
			if(next->type == T_LPAR){
				...
				E("cmd: Syntax error - expected funcCall, array index or assign");
				exit(synt);
			}		
			break;
	}
...
// spinavy hack ktory riesi problem old_cur := cur();
	switch((*cur)->type){
		...
		case T_KW_SORT:
		case T_KW_LENGTH:
			// parseCmd vrati nasledujici token
			cmd = parseCommand(cur);
